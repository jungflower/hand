...
int main( int argc, const char** argv)
{
    ...
    Mat img_gray;
    Mat img_roi;
    namedWindow("Original_image",cv::WINDOW_AUTOSIZE);
    namedWindow("Gray_image",cv::WINDOW_AUTOSIZE);
    namedWindow("Thresholded_image",cv::WINDOW_AUTOSIZE);
    ...
    while(1){
        ...
        findContours(img_threshold,contours,hierarchy,cv::RETR_EXTERNAL,cv::CHAIN_APPROX_SIMPLE,Point());
        if(contours.size()>0){
                ...
                for(size_t i=0;i<contours.size();i++){
                    if(contourArea(contours[i])>5000){
                        ...
                        if(indexOfBiggestContour==i){
                            ...
                            putText(img,a,Point(70,70),cv::FONT_HERSHEY_SIMPLEX,3,Scalar(255,0,0),2,8,false);
